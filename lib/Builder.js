...
const Builder = {
  ...
  demo: () => {
    ...
    const setIconBaseClass = OPTIONS.svg2font.iconBaseClass ?? iconBaseClass;
    console.log("setIconBaseClass=> ", setIconBaseClass);
    for (const i in UnicodeObj) {
      ...
      _codeHtml += `<li><i class="${setIconBaseClass}">${_code}</i><p>${i}：<br/> ${_code.replace(
        "&",
        "&amp;"
      )}</p></li>`;
      _classHtml += `<li><i class="${setIconBaseClass} icon-${i}"></i><p>${setIconBaseClass} <br/> icon-<b>${i}</b></p></li>`;
      _classCss += `\r\n.icon-${i}:before { content: "\\${_num}"; }`;
    }

    const _CSS = DEMO_CSS.replace(fontNameReg, fontName).replaceAll(
      iconBaseClass,
      setIconBaseClass
    );
   ...
   */
  init: ({ fontName, src, dist, startNumber, debug, svg2font }) => {
