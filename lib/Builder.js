...
const Builder = {
  ...
  svg: () => {
    ...
    function writeFontStream(fontStream, svgPath, iconPre) {
      ...
      let iconName = `${iconPre}${iconPre && "-"}${_svgName}`;
      glyph.metadata = {
  ...
  demo: () => {
    ...
    for (const i in UnicodeObj) {
      ...
      _codeHtml += `<li><i class="u-iconfont">${_code}</i><p>${i}： ${_code.replace(
        "&",
        "&amp;"
      )}</p></li>`;
      _classHtml += `<li><i class="u-iconfont icon-${i}"></i><p>${i}： .icon-${i}</p></li>`;
